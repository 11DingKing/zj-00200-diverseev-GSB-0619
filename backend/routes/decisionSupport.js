...
function simulatePolicyYears(baseline, params, options = {}) {
  const {
  ...
  for (let year = 1; year <= simulationYears; year++) {
    ...
    const scenarioCoverage = SCENARIOS.map((scenario) => {
      ...
      if (detailed) {
        entry.improvement = parseFloat(improvement.toFixed(1));
      }
      return entry;
    });
      ...
      ? {
          ...
          changes: {
            ...
            averageWeight: parseFloat(
              (
                ((newAvgWeight - baseline.averageWeight) /
...
router.post("/simulate/policy", async (req, res) => {
  try {
    ...
    const results = simulatePolicyYears(baseline, params, { detailed: true });
...
async function runSimulationForReport(params) {
  const baseline = await getCurrentBaseline();
  const results = simulatePolicyYears(baseline, params, { detailed: false });
