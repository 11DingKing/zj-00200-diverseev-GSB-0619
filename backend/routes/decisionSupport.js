...
function simulatePolicyYears(baseline, params) {
  const {
  ...
  const results = [];
  let currentState = {
    categoryDistribution: baseline.categoryDistribution.map((c) => ({ ...c })),
  };
...
router.post("/simulate/policy", async (req, res) => {
  try {
    ...
    const rawResults = simulatePolicyYears(baseline, {
    ...
    const results = rawResults.map((r) => {
      const yearFactor = r.year / simulationYears;
      const microSubsidyEffect = microVehicleSubsidy * 0.25 * yearFactor;
      const heavyRestrictionEffect = heavyVehicleRestriction * 0.18 * yearFactor;

      const scenarioCoverage = r.scenarioCoverage.map((s) => {
        let improvement = 0;
        if (s.scenario === "城市通勤") {
          improvement =
            microSubsidyEffect * 0.3 + lightweightMaterialAdoption * 0.15;
        } else if (s.scenario === "长途") {
          improvement = batteryEnergyDensityImprovement * 0.4;
        } else if (s.scenario === "载货") {
          improvement = newEnergyVehicleIncentive * 0.2;
        }
        return {
          ...s,
      ...
      const concentrationLevel =
        r.heavyRatio > 55 || r.largeCategoryRatio > 60
          ? "高度集中"
          : r.heavyRatio > 40 || r.largeCategoryRatio > 50
            ? "中度集中"
            : "分布均衡";
      ...
      return {
        ...r,
        scenarioCoverage,
        yearLabel: `第${r.year}年`,
        concentrationLevel,
        changes: {
          totalVehicles: parseFloat(
            (
              ((r.totalVehicles - baseline.totalVehicles) / baseline.totalVehicles) *
              100
            ).toFixed(1),
          ),
          averageWeight: parseFloat(
            (
              ((r.averageWeight - baseline.averageWeight) /
                baseline.averageWeight) *
              100
            ).toFixed(1),
          ),
          lightweightRatio: parseFloat(
            (r.lightweightRatio - baseline.lightweightRatio).toFixed(1),
          ),
          diversityScore: parseFloat(
            (r.diversityScore - baseline.diversityScore).toFixed(1),
          ),
        },
      };
    });
...
async function runSimulationForReport(params) {
  const baseline = await getCurrentBaseline();
  const results = simulatePolicyYears(baseline, params);
